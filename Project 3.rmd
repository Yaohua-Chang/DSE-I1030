...
reg.summary$rsq
reg.summary$which
# The output indicates that the best two-variables for the model are BT_M15 and BT_M16.
...
#Model1 and Model2 are identical modelsand their adjusted R squared is identical. 
#They have a near perfect value (1) Adj R squared value.
...
# Question 4
#Estimate the coefficients using shrinkage
#methods (Lasso and Ridge regression). Evaluate and compare with operational Ts
...
#number of terms without significant loss of performance. Compare with Lasso
#and Ridge results. What is the best model obtained according Cp, AIC, BIC, and
#adjusted R2
...
#Question 6
#When the number of features used in a model increases, the training error will
#necessarily decrease, but the test error may not. Perform best subset selection on
#the training set (one of the matchup files), and plot the training set (using another
#matchup file) RMSE associated with the best model of each size. For which
#model does the test set MSE take on its minimum value? How does the model at
#which the test set MSE is minimized compare to the model used to generate the
#data? Does this agree with the coefficient values of the model?
...
#Question 7
#Estimate the test error of your best model using the validation set approach (kfold
#and/or bootstrap). Does it agree with your findings in part 6? Comment on the result
