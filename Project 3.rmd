...
---
<div id="instructions">
Due at 6:00 pm on Thursday, December 20, 2018<br />
Name: Yaohua Chang<br />
Name: Jiffar
</div>
...
## Question 1
Use subset selection methods to find the best 𝜆! and 𝜆! in equation (1). Since there are only 2 variables, 𝑇!! and 𝑇!! , exhaustive search is an appropriate method.
```{r}
...
##Question 5
Starting from the form (3n) and (3d), use backward selection method to reduce number of terms without significant loss of performance. Compare with Lasso and Ridge results. What is the best model obtained according Cp, AIC, BIC, and adjusted R2? Show some plots to provide evidence for your answer, and report the coefficients of the best model obtained.
```{r }
...
Conslusion: The result tells us that only keeping b1,b10 and remove other variables is without significant loss of performance
...
##Question 6
When the number of features used in a model increases, the training error will necessarily decrease, but the test error may not. Perform best subset selection on the training set (one of the matchup files), and plot the training set (using another matchup file) RMSE associated with the best model of each size. For which model does the test set MSE take on its minimum value? How does the model at which the test set MSE is minimized compare to the model used to generate the data? Does this agree with the coefficient values of the model?
```{r}
...
## Question 7
Estimate the test error of your best model using the validation set approach (k- fold and/or bootstrap). Does it agree with your findings in part 6? Comment on the result.
